...
""")
parser.add_argument("--window_step_size", type=int, required=False, help="step size for windowed files")
    ...
    def write_wig(self, chromsomes):
        with open(self.output, 'w') as f:
            if self.type =="fst" or self.type == "pi":
                ...
                for chrom in chromsomes:
                    f.write(f"fixedStep chrom={chrom} start=1 step={self.window_step_size} span={self.window_step_size - 1}\n")
                    for value in chromsomes[chrom]:
                        f.write(f"{value}\n")
            else:
    ...
    def convert_tajimas_d_to_wig(self):
        ...
        with open(self.input,'r') as f:
            ...
            for line in f:
                ...
                chromsomes[chrom].append(tajimas_d)
        self.write_wig(chromsomes)
